...
Drought_LT_CPUE<-readRDS("Outputs/Drought_LT_CPUE.rds")
#join distances back into Drought dataset
Drought_LT_dist<-Drought_LT_CPUE%>%
...
Drought_dist_tot<-Drought_LT_dist%>%group_by(Station,Date,SubRegion,water_year,month,Season,distance_km)%>%
  filter(water_year>1974)%>%
  dplyr::summarise(totalCPUE=sum(CPUE))

#calculate center of distributions for total CPUE
ddt_grouped<-Drought_dist_tot%>%group_by(Station,distance_km,Date,SubRegion,water_year,month,Season)%>%
  filter(water_year>1974)%>%
  dplyr::summarise(sumCPUE=sum(totalCPUE))
ddt_grouped<-ddt_grouped%>%
  group_by(water_year,month,SubRegion,Season,Station,distance_km)%>%
...
ddtaxa_grouped<-ddtaxa%>%group_by(Station,distance_km,Date,SubRegion,water_year,month,Season,Taxlifestage)%>%
  dplyr::summarise(sumCPUE=sum(CPUE))
ddtaxa_grouped<-ddtaxa_grouped%>%
  group_by(water_year,month,SubRegion,Season,Station,distance_km,Taxlifestage)%>%
  dplyr::summarise(month_CPUE=mean(sumCPUE))
ddtaxa_grouped<-ddtaxa_grouped%>%
  inner_join(rosies_table)%>%
  filter(Long_term==TRUE)
#calculate seasonal averages
ddtaxa_grouped<-ddtaxa_grouped%>%
  group_by(water_year,SubRegion,Season,Station,distance_km,Taxlifestage)%>%
  dplyr::summarise(s_CPUE=mean(month_CPUE))
ddtaxa_grouped<-ddtaxa_grouped%>%
  group_by(Season,water_year,Station,distance_km,Taxlifestage)%>%
  dplyr::summarise(szn_CPUE=mean(s_CPUE))
ddtaxa_grouped$freq<-as.integer(ddtaxa_grouped$szn_CPUE)
ddtaxa_grouped$freq<-ifelse(ddtaxa_grouped$szn_CPUE>0 & ddtaxa_grouped$szn_CPUE<1,1,ddtaxa_grouped$freq)
